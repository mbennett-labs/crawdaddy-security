...
// x402 payment config
const WALLET_PATH = path.join(process.env.HOME || "/home/ubuntu", ".automaton", "wallet.json");
...
function loadPaymentWallet(): ethers.Wallet {
  if (_cachedWallet) return _cachedWallet;
  const raw = fs.readFileSync(WALLET_PATH, "utf-8");
  const { privateKey } = JSON.parse(raw);
  _cachedWallet = new ethers.Wallet(privateKey);
  return _cachedWallet;
}

async function signX402Payment(req: X402PaymentRequirement): Promise<string> {
  const wallet = loadPaymentWallet();
  ...
  // USDC on Base: name "USD Coin", version "2", chainId 8453
  const domain = {
    name: "USD Coin",
    version: "2",
    chainId: 8453,
...
async function fetchWithX402(url: string, timeoutMs: number = 30000): Promise<any> {
  ...
  try {
    ...
    if (response.status === 402) {
      ...
      const accepts: X402PaymentRequirement[] = body.accepts || [];
      const req = accepts.find((a) => a.scheme === "exact" && a.network === "base") || accepts[0];
      if (!req) throw new Error("402 Payment Required but no accepted payment scheme found");

      console.log(`[x402] Paying ${req.maxAmountRequired} atomic USDC to ${req.payTo}`);
      const paymentHeader = await signX402Payment(req);
      ...
      try {
        ...
        if (!paidResponse.ok) {
          const errText = await paidResponse.text().catch(() => paidResponse.statusText);
          throw new Error(`API returned ${paidResponse.status} after x402 payment: ${errText}`);
        }
...
export async function executeJob(request: any): Promise<ExecuteJobResult> {
  ...
  try {
    ...
    if (scanType === "token") {
      ...
      const tokenData = await fetchWithX402(
      ...
      try {
        honeypotData = await fetchWithX402(
      ...
      // Wallet risk scan
      const walletData = await fetchWithX402(
      ...
      const honeypotData = await fetchWithX402(
        `${API_BASE}/api/honeypot/check?address=${address}&chain=${chain}`
      );
